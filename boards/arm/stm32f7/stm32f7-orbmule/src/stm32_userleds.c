...
{
  if (led & 1) stm32_gpiowrite(GPIO_LD1, ledon);
  if (led & 2) stm32_gpiowrite(GPIO_LD2, ledon);
  if (led & 4) stm32_gpiowrite(GPIO_LD3, ledon);
}
...
{
  stm32_gpiowrite(GPIO_LD1, ledset & 1);
  stm32_gpiowrite(GPIO_LD2, ledset & 2);
  stm32_gpiowrite(GPIO_LD3, ledset & 4);
}
